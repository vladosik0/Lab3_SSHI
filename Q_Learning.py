class QLearnAgent(Agent):
  ...
  def __init__(self, alpha=0.6, epsilon=0.1, gamma=0.8, numTraining=10):
    # alpha       - learning rate
  ...
  def update_Q_value(self, state, final_step=False):
    ...
    self.Q_values[self.prev_state][self.prev_action] += (
              self.alpha * (reward + self.gamma * max_Q_value - self.Q_values[self.prev_state][self.prev_action]))
